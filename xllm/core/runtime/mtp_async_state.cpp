...
bool can_use_combined_decode(bool enable_schedule_overlap,
                             bool supports_device_target_context,
                             bool supports_combined_draft_attention,
                             bool has_recurrent_layers,
                             bool enable_atb_spec_kernel,
                             bool draft_is_eager,
                             int32_t dp_size) {
  return enable_schedule_overlap && supports_device_target_context &&
         supports_combined_draft_attention && !has_recurrent_layers &&
         !enable_atb_spec_kernel && draft_is_eager && dp_size <= 1;
}

AcceptedState build_accepted_state(
...
torch::Tensor select_rows(const torch::Tensor& values,
  ...
  torch::Tensor index =
      selected_rows
          .to(torch::dtype(torch::kLong).device(values.device()))
          .contiguous();

...
torch::Tensor run_causal_conv1d_graph_update(
  ...
  auto event = std::make_shared<xllm::npu::NPUExternalEvent>();
  event->block(stream);
  event->reset(stream);
...
torch::Tensor Qwen3GatedDeltaNetBaseImpl::forward(
  ...
  auto graph_context = input_params.graph_buffer.acl_graph_task_update_context;
  ...
  if (!use_spec_verify && is_any_prefill) {
    ...
    if (register_conv1d_graph_update) {
      ...
      xllm::kernel::causal_conv1d_out(
          ...
          std::optional<torch::Tensor>(),
          torch::IntArrayRef(input_params.query_start_loc),
          torch::IntArrayRef(input_params.linear_state_indices_host),
